...
You can reject requests that match patterns for:
- **SQL injection** attacks
- **Cross-Site Scripting (XSS)** attacks
- **IP-based filtering** - Block specific IP addresses
- **Geographic filtering** - Block requests from specific countries
- **Request size limits** - Block overly large requests
- **Bad bots and content scrapers** - Filter based on user-agents
- **Malformed requests** - Validate request structure

### How WAF Works

You can filter web requests based on:
- IP address
- Geographic area
- Request size
- String or regular expression patterns

These conditions can be applied to:
- HTTP headers
- Request body
- URI path

This allows you to create complex rules to block attacks from specific user-agents, bad bots, or content scrapers.

## Managed Rules

You can take advantage of **Managed Rules** from AWS Marketplace to get immediate protections for your APIs from common threats, such as:
- **OWASP Top 10** security risks
- **Common Vulnerabilities and Exposures (CVE)**
- Pre-built rules ready to deploy
- Regularly updated by AWS and partners

## Module Goal

In this module, you will create a WAF ACL (Access Control List) and attach it to the API Gateway we created.
