...
1. Monolithic applications typically have centralized permissions in code

2. Distributed microservices spread permissions and logs across multiple services

3. This distributed model loses central control and makes auditing difficult

4. Authorization-as-a-service regains centralized access control and logging

5. It enables iteration on least privilege principles

6. It implements zero trust architecture

7. It makes applications audit-friendly and AI-ready

## What is Amazon Verified Permissions?

8. AVP is an authorization-as-a-service solution

9. It replaces custom application-specific authorization systems

10. Applications or enforcement points (like API Gateway) call AVP to check access

11. AVP determines if a user can access a resource based on policies

## Key Features

12. **Workload Support**: Works with serverless, containers, applications, databases, and gateways

13. **Policies-as-Code**: Deploy policy updates with code in CI/CD pipelines

14. **Identity Provider Support**: Works with Amazon Cognito and OIDC/OAuth providers

15. **Policy Information Points**: Add context to inform policy decisions

16. **Audit Ready**: Outputs logs to CloudTrail (policy model changes) and CloudWatch (policy requests)

  ![](../img/module-3-verified-permissions/avp-features.png)

## Architecture

17. Partner Admin API uses AVP Quick Start for rapid authorization enforcement

18. The architecture includes:
...
1. Add the Lambda function event for AVP under ManagePartnerFunction:
...
1. Add outputs to the template file at the end:
...
1. Save the `src/template.yaml` file
...
1. Wait for the stack deployment to complete

  ![](../img/module-3-verified-permissions/deployment-complete.png)
...
1.  Copy the API endpoint URL to your notes file
...
2.  Scroll down to the **App clients** section

3.  Copy the **Client ID** of the **avp-admin-client**

4.  Save the Client ID to your notes file
...
1.  Enter a username (e.g., username)

2.  Enter a password that meets the password requirements (e.g., Username*)

3.  Click **Create user**

  ![](../img/module-3-verified-permissions/2026-01-29-09-00-22.png)

4.  User is created with "Force change password" status

![](../img/module-3-verified-permissions/2026-01-29-09-01-51.png)

5.  Run the following command to set a permanent password:
...
1.  Click on the newly created user to open the user detail page

2.  Scroll down to the **Group memberships** section

3.  Click **Add user to a group**

![](../img/module-3-verified-permissions/2026-01-29-09-06-55.png)

4.  Select the **admin-users** group
...
33. Repeat steps 22-26 to create another user (e.g., username2)
...
6. Review **Deploy app integration** and click **Create policy store**
