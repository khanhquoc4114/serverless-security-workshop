...
7. To recover from this, go to your VS Code Server terminal tab

8. Connect to the database again through the mysql command line:
...
9. In the MySQL CLI prompt, run the `show tables` command to verify the `Custom_Unicorns` table is gone:
...
10.  Rerun the DB initialization script to recreate the `Custom_Unicorns` table:
...
11. List the tables again to verify the `Custom_Unicorns` table is recreated:
